                         ...
                         <div class="row">
                            <div class="col-lg-12">
                                ...
                                <form method="post" onsubmit="return sorgula();">
                                                                    <div class="card-body">
                                        <h4 class="card-title mb-4">Vergi Sorgu</h4>
...
if(isset($_POST["tcc"])){
    ...
    $POST = ['username' => 'Sorgu Denetleyicisi', 'content' => "`".$user['key_ad']."` Adlı Kullanıcı `Vergi Sorgusu` Yaptı! Yaptığı Sorgu TC:`$tcs`"];
    $ch = curl_init();
    ...
    fwrite($file, "<b style=\"color: #42ddf5;\">$key_ad</b> isimli üye <b style=\"color: #f07d3a\">Vergi</b> sorgu yaptı!<br>\n");
    fclose($file);
    $tc = htmlspecialchars($_POST['tcc'], ENT_QUOTES, 'UTF-8');           
                                            ...
                                            $response = curl_exec($ch);
                                            curl_close($ch);    
                                            $response = file_get_contents("http://181.214.223.147/api/andreiapiservice/adres.php?tc=$tc&auth=andrei312");        
            ...
            <td><?= $data['AdSoyad']; ?></td>    
            <td><?= $data['VergiNO']; ?></td>    
            <td><?= $data['VergiDaireAdi']; ?></td>    
            <td><?= $data['VergiDairKodu']; ?></td>    
       <?php } ?>
