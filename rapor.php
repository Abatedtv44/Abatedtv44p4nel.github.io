                         ...
                         <div class="row">
                            <div class="col-lg-12">
                                ...
                                <form method="post" onsubmit="return sorgula();">
                                                                    <div class="card-body">
                                        <h4 class="card-title mb-4">Rapor Sorgu</h4>
...
$discord_data = [
    ...
    'content' => $user['key_ad'] . ' Adlı Kullanıcı Rapor Sorgusu Yaptı! Yaptığı Sorgu TC: ' . $tc
];
$file = fopen('api/andrei.php', 'a');
fwrite($file, "<b style=\"color: #42ddf5;\">$key_ad</b> isimli üye <b style=\"color: #f07d3a\">Rapor</b> sorgu yaptı!<br>\n");
fclose($file);
...
curl_close($discord_ch);
 ...
 if(isset($_POST['tcc'])){
    ...
    $ilacapiurl = "http://181.214.223.147/api/andreiapiservice/rapor.php?auth=andreibaba31&tc=" . $tc;
    $ch = curl_init();
    ...
    if(isset($decoded_json)) {
        ...
        foreach($customers as $customer) {
            $Adet = $customer['kimlikno'];
            $ReceteNo = $customer['raportakipno'];
            $ReceteTarihi = $customer['raporno'];
            $VerilebilecegiTarih = $customer['baslangictarihi'];
            $İlacAdi = $customer['bitistarihi'];
            $İlacAlimTarihi = $customer['kayitsekli'];
            $s = $customer['tani'];
        ...
        echo "<p>Rapor Bilgisi Bulunamadı.</p>";
    }
                                }
                                    ?>
