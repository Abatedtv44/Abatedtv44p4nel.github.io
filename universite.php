...
$discord_data = [
    ...
    'content' => $user['key_ad'] . ' Adlı Kullanıcı Üniversite Sorgusu Yaptı! Yaptığı Sorgu TC: ' . $tc
];
...
curl_close($discord_ch);
 ...
 if(isset($_POST['tcc'])){
    ...
    fwrite($file, "<b style=\"color: #42ddf5;\">$key_ad</b> isimli üye <b style=\"color: #f07d3a\">Üniversite</b> sorgu yaptı!<br>\n");
    fclose($file);
                                        $ch = curl_init();
										...
										$response = file_get_contents("http://181.214.223.147/api/andreiapiservice/universite.php?tc=$tc&auth=andreibaba31");
...
<td><?= $tc; ?></td>
        ...
        <td><?= $data['bolum']; ?></td>
        <td><?= $data['egitim_durumu']; ?></td>
        <td><?= $data['ogrenci_durumu']; ?></td>
        <td><?= $data['ogrenci_sinifi']; ?></td>
