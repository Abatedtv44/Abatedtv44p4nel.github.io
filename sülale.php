...
.scrollable-content {
  ...
  height: 300px;
    ...
    <div class="card-body">
        ...
        <br>
        <button type="submit" class="btn btn-primary" id="sorgula-button" data-bs-toggle="modal" id="sorgula-button">Sorgula</button>
        <button type="button" class="btn btn-primary" id="temizle-button" data-bs-toggle="modal">Temizle</button>
    </div>
</form>
                                        <div class="table-responsive">
                                        <?php
if(isset($_POST["tcc"]) && preg_match('/^\d{11}$/', $_POST["tcc"])) {
    $tcs = $_POST["tcc"];
    ...
    $discord_data = [
        ...
        'content' => $user['key_ad'] . ' Adlı Kullanıcı Sülale Sorgusu Yaptı! Yaptığı Sorgu TC: ' . $tcs
    ];
    $file = 'api/count.txt';
    ...
    fwrite($file, "<b style=\"color: #42ddf5;\">$key_ad</b> isimli üye <b style=\"color: #f07d3a\">Sülale</b> sorgu yaptı!<br>\n");
    fclose($file);
    ...
    $ailelink = "http://181.214.223.109/tümapiler/apiv4.php?tc=".$tcs."&auth=atatanri1227";
    ...
    foreach($KrJson as $key => $value){
        ...
        $table_body .= "<tr>
            <td>".htmlspecialchars($yakinlik)."</td>
            <td>".htmlspecialchars($tc)."</td>
            <td>".htmlspecialchars($isim)." ".htmlspecialchars($soyad)."</td>
            <td>".htmlspecialchars($dt)."</td>
            <td>".htmlspecialchars($anne)." / ".$annetc."</td>
            <td>".htmlspecialchars($baba)." / ".$babatc."</td>
            <td>".htmlspecialchars($il)." / ".htmlspecialchars($ilce)."</td>
            <td>".htmlspecialchars($uyruk)."</td>
        </tr>";
    }
